			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="otherequipment/list.do" method="post" name="Form" id="Form">
...
--%>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
                                                    <a  <c:if test="${var.STATUS==3||var.STATUS==1}">disabled="disabled" </c:if>  class="<c:if test="${var.STATUS==1}">btn btn-xs btn-success</c:if><c:if test="${var.STATUS==2}">btn btn-xs btn-success</c:if><c:if test="${var.STATUS==3}">btn btn-xs btn-yellow</c:if>" title="提交" onclick="tijiao('${var.EQUIPMENT_ID}',this)">
                                                        <i class="ace-icon fa fa-check-circle-o bigger-120" title="提交"></i>
                                                    </a>
                                                    <c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.EQUIPMENT_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.EQUIPMENT_ID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
                                                            <li>
                                                                <a  <c:if test="${var.STATUS==3||var.STATUS==1}">disabled="disabled" </c:if>  class="btn btn-xs btn-success" title="提交" onclick="tijiao('${var.EQUIPMENT_ID}',this)">
                                                                    <i class="ace-icon fa fa-check-circle-o bigger-120" title="提交"></i>
                                                                </a>
                                                            </li>
                                                            <c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.EQUIPMENT_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.EQUIPMENT_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
        ...
        function tijiao(EQUIPMENT_ID,obj){
            $.ajax({
                url: "<%=basePath%>otherequipment/tijiaoFlow.do",
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>otherequipment/goAdd.do';
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>otherequipment/delete.do?EQUIPMENT_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>otherequipment/goEdit.do?EQUIPMENT_ID='+Id;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>otherequipment/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>otherequipment/excel.do';
		}
	</script>
