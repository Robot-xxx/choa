            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <!-- 检索  -->
                        <form action="payrequest/list.do" method="post" name="Form" id="Form">
                                                   ...
                                                   <%-- <td class='center'>${var.PAY_ACCOUNT}</td>
                                                    ...
                                                    <td class="center">
                                                        ...
                                                        <div class="hidden-sm hidden-xs btn-group">
                                                            ...
                                                            <a class="btn btn-xs btn-success" title="打印"
                                                               onclick="dayin('${var.REQUEST_ID}');">
                                                                <i class="ace-icon fa fa-print bigger-120"
                                                                   title="打印"></i>
                                                            </a>
                                                            <c:if test="${QX.edit == 1 }">
                                                                <a class="btn btn-xs btn-success" title="编辑"
                                                                   onclick="edit('${var.REQUEST_ID}');">
...
<script type="text/javascript">
    ...
    function tijiao(REQUEST_ID, obj) {
        $.ajax({
            url: "<%=basePath%>payrequest/tijiaoFlow.do",
    ...
    function dayin(requestid) {

        $.ajax({
            url: "<%=basePath%>payrequest/YanZheng.do",
            ...
            success: function (data) {
                if(data.tag==1){
                    ...
                    diag.Drag = true;
                    diag.Title = "新增";
                    diag.URL = '<%=basePath%>payrequest/goPrint.do?REQUEST_ID=' + requestid;
                    diag.Width = 900;
                    diag.Height = 600;
    ...
    function add() {
        ...
        diag.Drag = true;
        diag.Title = "新增";
        diag.URL = '<%=basePath%>payrequest/goAdd.do';
        diag.Width = 800;
        diag.Height = 600;
        diag.Modal = true;				//有无遮罩窗口
        diag.ShowMaxButton = true;	//最大化按钮
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "<%=basePath%>payrequest/delete.do?REQUEST_ID=" + Id + "&tm=" + new Date().getTime();
    ...
    function edit(Id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>payrequest/goEdit.do?REQUEST_ID=' + Id;
        diag.Width = 800;
        diag.Height = 600;
        diag.Modal = true;				//有无遮罩窗口
        diag.ShowMaxButton = true;	//最大化按钮
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>payrequest/deleteAll.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
