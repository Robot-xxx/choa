            ...
            <div class="page-content">
                <div class="row">
                    <div class="col-xs-12">
                        ...
                        <!-- 检索  -->
                        <form action="identifymanagement/list.do" method="post" name="Form" id="Form">
                            <table style="margin-top:5px;">
                                <tr>
                                    <td>
                                        <div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="这里输入项目编号" class="nav-search-input"
                                                   id="nav-search-input" autocomplete="off" name="keywords"
                                                   value="${pd.keywords }" placeholder="这里输入项目编号"/>
                                                ...
                                                <tr>
                                                    <td class='center'>
                                                        <label class="pos-rel"><input type='checkbox' name='ids'
                                                                                      value="${var.IDENTIFYMANAGEMENT_ID}"
                                                                                      class="ace"/><span
                                                                class="lbl"></span></label>
                                                    </td>
                                                    <td class='center'
                                                    ...
                                                    <td class='center'><a class="btn btn-mini btn-purple" onclick="addRenKuan('${var.RENKUAILEIXING}','${var.WEILINGJINE}','${var.IDENTIFYMANAGEMENT_ID}','${var.PROJECT_ID}','${var.PROJECT_NAME}')">
                                                        ...
                                                        <div class="hidden-sm hidden-xs btn-group">
                                                            ...
                                                            <c:if test="${QX.edit == 1 }">
                                                                <a class="btn btn-xs btn-success" title="编辑"
                                                                   onclick="edit('${var.IDENTIFYMANAGEMENT_ID}');">
                                                            ...
                                                            <c:if test="${QX.del == 1 }">
                                                                <a class="btn btn-xs btn-danger"
                                                                   onclick="del('${var.IDENTIFYMANAGEMENT_ID}');">
...
<script type="text/javascript">
    ...
    function del(Id) {
        bootbox.confirm("确定要删除吗?", function (result) {
            if (result) {
                top.jzts();
                var url = "<%=basePath%>identifymanagement/delete.do?IDENTIFYMANAGEMENT_ID=" + Id + "&tm=" + new Date().getTime();
    ...
    function edit(Id) {
        ...
        diag.Drag = true;
        diag.Title = "编辑";
        diag.URL = '<%=basePath%>identifymanagement/goEdit.do?IDENTIFYMANAGEMENT_ID=' + Id;
        diag.Width = 450;
        diag.Height = 355;
        diag.Modal = true;				//有无遮罩窗口
        diag.ShowMaxButton = true;	//最大化按钮
            ...
            if (result) {
                ...
                if (str == '') {
                    ...
                    if (msg == '确定要删除选中的数据吗?') {
                        ...
                        $.ajax({
                            type: "POST",
                            url: '<%=basePath%>identifymanagement/deleteAll.do?tm=' + new Date().getTime(),
                            data: {DATA_IDS: str},
                            dataType: 'json',
    ...
    //导出excel
    function toExcel() {
        window.location.href = '<%=basePath%>identifymanagement/excel.do';
    }
</script>
