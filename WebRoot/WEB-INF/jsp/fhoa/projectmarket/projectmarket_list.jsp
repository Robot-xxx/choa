			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="projectmarket/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								<td>
									<div class="nav-search">
										<span class="input-icon">
											<input type="text" placeholder="这里输入公司名称、项目编号、项目名称" class="nav-search-input" id="nav-search-input" autocomplete="off" name="keywords" value="${pd.keywords }" placeholder="这里输入系统编序号"/>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.PROJECT_MARKET_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
...
<%--
											<td class='center'>${var.CUMULATIVE_BILLING}</td>
--%>
                                            ...
                                            <td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.PROJECT_MARKET_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.PROJECT_MARKET_ID}');">
												...
												<div class="hidden-md hidden-lg">
													<div class="inline pos-rel">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="edit('${var.PROJECT_MARKET_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
															...
															<c:if test="${QX.del == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="del('${var.PROJECT_MARKET_ID}');" class="tooltip-error" data-rel="tooltip" title="删除">
																	<span class="red">
	...
	<script type="text/javascript">
        ...
        function tijiao(PROJECT_MARKET_ID,obj){
            $.ajax({
                url: "<%=basePath%>projectmarket/tijiaoFlow.do",
                data:{'PROJECT_MARKET_ID':PROJECT_MARKET_ID},
        ...
        //新增
        function selectHuiKuai(PROJECT_MARKET_ID){

            top.jzts();
            var diag = new top.Dialog();
            diag.Drag=true;
            diag.Title ="新增";
            diag.URL = '<%=basePath%>markethuikuai/list.do?PROJECT_MARKET_ID='+PROJECT_MARKET_ID;
            diag.Width = 800;
            diag.Height = 600;
            diag.Modal = true;				//有无遮罩窗口
            diag. ShowMaxButton = true;	//最大化按钮
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>projectmarket/goAdd.do';
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>projectmarket/delete.do?PROJECT_MARKET_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>projectmarket/goEdit.do?PROJECT_MARKET_ID='+Id;
			 diag.Width = 800;
			 diag.Height = 600;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>projectmarket/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>projectmarket/excel.do';
		}
	</script>
