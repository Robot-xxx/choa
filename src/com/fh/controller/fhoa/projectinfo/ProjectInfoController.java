package com.fh.controller.fhoa.projectinfo;
...
import com.fh.service.fhoa.projectinfo.ProjectInfoManager;
...
public class ProjectInfoController extends BaseController {
	...
	public ModelAndView list(Page page) throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		String keywords = pd.getString("PARENT_ID");				//关键词检索条件
		if(null != keywords && !"".equals(keywords)){
			pd.put("PARENT_ID", keywords.trim());
		}
		page.setPd(pd);
		List<PageData>	varList = projectinfoService.list(page);	//列出ProjectInfo列表
		mv.setViewName("fhoa/projectinfo/projectinfo_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("fhoa/projectinfo/projectinfo_edit");
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var2", varOList.get(i).get("PROJECTNUMBER").toString());	//2
			vpd.put("var3", varOList.get(i).getString("PROJECTPRICE"));	    //3
			varList.add(vpd);
		}
