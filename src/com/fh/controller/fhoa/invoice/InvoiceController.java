package com.fh.controller.fhoa.invoice;
...
import com.fh.service.fhoa.invoice.InvoiceManager;
...
public class InvoiceController extends BaseController {
	...
	public ModelAndView list(Page page) throws Exception{
		...
		String ticket_id = pd.getString("TICKET_ID");				//关键词检索条件
		if(null != keywords && !"".equals(keywords)){
			pd.put("keywords", keywords.trim());
		}
		if(null != ticket_id && !"".equals(ticket_id)){
			pd.put("ticket_id", ticket_id.trim());
		}
		page.setPd(pd);
		List<PageData>	varList = invoiceService.list(page);	//列出Invoice列表
		mv.setViewName("fhoa/invoice/invoice_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("fhoa/invoice/invoice_edit");
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var1", varOList.get(i).getString("BY_STAGES"));	    //1
			vpd.put("var2", varOList.get(i).getString("INVOICENUMBER"));	    //2
			vpd.put("var3", varOList.get(i).getString("NAME"));	    //3
			vpd.put("var4", varOList.get(i).get("MONEY").toString());	//4
			varList.add(vpd);
		}
